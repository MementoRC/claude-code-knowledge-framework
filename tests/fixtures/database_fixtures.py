...
class DummyChromaDBConnector:
    ...
    def __init__(self):
        self.collections = {"code_patterns": [], "error_solutions": []}
        self.added_docs = []
    ...
    def update_document(
        self, collection_name, doc_id, document=None, embedding=None, metadata=None
    ):
        for doc in self.collections[collection_name]:
            if doc["id"] == doc_id:
    ...
    def delete_document(self, collection_name, doc_id):
        ...
        self.collections[collection_name] = [
            doc for doc in self.collections[collection_name] if doc["id"] != doc_id
        ]
...
class DummyPostgreSQLConnector:
    ...
    def update_record(
        self, model_class: Any, record_id: str, updates: dict[str, Any]
    ) -> bool:
        table_name = model_class.__tablename__
        ...
        if record_id in self.tables[table_name]:
            ...
            if table_name == "patterns":
                ...
                self.tables["pattern_category_links"] = {
                    k: v
                    for k, v in self.tables["pattern_category_links"].items()
                    if v[0] != record_id
                }
    ...
    def get_all_records(
        self, model_class: Any, limit: Optional[int] = None
    ) -> list[dict[str, Any]]:
        table_name = model_class.__tablename__
    ...
    def filter_records(
        self, model_class: Any, filters: dict[str, Any], limit: Optional[int] = None
    ) -> list[dict[str, Any]]:
        table_name = model_class.__tablename__
        results = []
    ...
    def add_pattern_to_category(self, pattern_id: str, category_id: str) -> bool:
        ...
        if link_id in self.tables["pattern_category_links"]:
            self._logger.debug(
                f"Dummy PG: Link between pattern {pattern_id} and category {category_id} already exists."
            )
            return True  # Idempotent
        self.tables["pattern_category_links"][link_id] = (pattern_id, category_id)
        self._logger.debug(
            f"Dummy PG: Linked pattern {pattern_id} to category {category_id}"
        )
        return True
    ...
    def remove_pattern_from_category(self, pattern_id: str, category_id: str) -> bool:
        ...
        if link_id in self.tables["pattern_category_links"]:
            ...
            self._logger.debug(
                f"Dummy PG: Unlinked pattern {pattern_id} from category {category_id}"
            )
            return True
        self._logger.debug(
            f"Dummy PG: Link between pattern {pattern_id} and category {category_id} not found for removal."
        )
    ...
    def get_patterns_in_category(self, category_id: str) -> list[str]:
        return [
            link[0]
            for link in self.tables["pattern_category_links"].values()
            if link[1] == category_id
        ]
    ...
    def get_categories_for_pattern(self, pattern_id: str) -> list[dict[str, Any]]:
        category_ids = [
            link[1]
            for link in self.tables["pattern_category_links"].values()
            if link[0] == pattern_id
        ]
        # Need to mock the model classes for get_record to work with them
        mock_pattern_category_model = type(
            "PatternCategory", (object,), {"__tablename__": "pattern_categories"}
        )
        return [
            self.get_record(mock_pattern_category_model, cid)
            for cid in category_ids
            if self.get_record(mock_pattern_category_model, cid)
        ]
...
class DummyUnifiedDatabase:
    ...
    def add_pattern(
        ...
        pg_metadata.update(
            {"id": pattern_id, "project_id": project_id, "document_text": document_text}
        )
        ...
        chroma_success = self.chroma_connector.add_document(
            "code_patterns", pattern_id, document_text, embedding, metadata
        )
    ...
    def update_pattern(
        ...
        pg_success = self.pg_connector.update_record(
            MockPattern, pattern_id, pg_updates
        )
        if not pg_success:
            return False
    ...
    def search_patterns(
        ...
        chroma_results = self.chroma_connector.search_documents(
            "code_patterns", query_embedding, n_results, min_similarity, metadata_filter
        )
        final_results = []
    ...
    def add_project(
        ...
        return self.pg_connector.add_record(
            MockProject,
            {
    ...
    def update_category(self, category_id: str, updates: dict[str, Any]) -> bool:
        return self.pg_connector.update_record(
            MockPatternCategory, category_id, updates
        )
    ...
    def add_error_solution(
        ...
        if not pg_success:
            return None
        chroma_success = self.chroma_connector.add_document(
            "error_solutions", solution_id, document_text, embedding, metadata
        )
        if not chroma_success:
    ...
    def add_team_access(
        ...
        return self.pg_connector.add_record(
            MockTeamAccess,
            {
                "id": access_id or str(uuid.uuid4()),
                "user_id": user_id,
                "project_id": project_id,
                "role": role,
            },
    ...
    def get_team_access_for_project(self, project_id: str) -> list[dict[str, Any]]:
        return self.pg_connector.filter_records(
            MockTeamAccess, {"project_id": project_id}
        )
    ...
    def update_compatibility_entry(
        ...
        return self.pg_connector.update_record(
            MockCompatibilityMatrix, entry_id, updates
        )
...
def db_state_manager(dummy_chromadb_connector, dummy_postgresql_connector):
    ...
    class DBStateManager:
        ...
        def snapshot(self):
            self.chroma_snapshots.append(
                copy.deepcopy(self.chroma_connector.collections)
            )

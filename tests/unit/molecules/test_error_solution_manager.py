...
def manager(mock_unified_db, mock_semantic_search):  # Updated argument
    return ErrorSolutionManager(
        mock_unified_db, mock_semantic_search
    )  # Updated constructor call


def test_initialization(mock_unified_db, mock_semantic_search):  # Updated argument
    mgr = ErrorSolutionManager(
        mock_unified_db, mock_semantic_search
    )  # Updated constructor call
...
def test_add_error_solution_success(
    manager, mock_unified_db, mock_semantic_search
):  # Updated argument
...
def test_add_error_solution_no_semantic(manager, mock_semantic_search):
    ...
    result = ErrorSolutionManager(
        MagicMock(spec=UnifiedDatabase), mock_semantic_search
    ).add_error_solution({"document": "err"})
    assert result is None
...
def test_get_error_solution_success(manager, mock_unified_db):  # Updated argument
    mock_unified_db.get_error_solution.return_value = {
        "id": "sol-1"
    }  # Updated mock method
    result = manager.get_error_solution("sol-1")
    assert result == {"id": "sol-1"}
    mock_unified_db.get_error_solution.assert_called_once_with(
        "sol-1"
    )  # Updated assertion
...
def test_search_error_solutions_success(
    manager, mock_unified_db, mock_semantic_search
):  # Updated argument
    mock_semantic_search.encode.return_value = [0.1, 0.2, 0.3]
    mock_unified_db.search_error_solutions.return_value = [
        {"id": "sol-1"}
    ]  # Updated mock method
...
def test_update_error_solution_success(
    manager, mock_unified_db, mock_semantic_search
):  # Updated argument
...
def test_update_error_solution_no_semantic(
    ...
    mock_unified_db.update_error_solution.assert_called_once_with(  # Updated assertion
        ...
        metadata={
            "updated_at": ANY
        },  # Only updated_at is added if metadata is not explicitly provided in updates
        project_id=None,
    )
...
def test_update_error_solution_metadata_only(
    manager, mock_unified_db
):  # Updated argument
    mock_unified_db.update_error_solution.return_value = True  # Updated mock method
...
def test_delete_error_solution_success(manager, mock_unified_db):  # Updated argument
    ...
    assert result is True
    mock_unified_db.delete_error_solution.assert_called_once_with(
        "sol-1"
    )  # Updated assertion

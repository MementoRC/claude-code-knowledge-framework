...
def test_analyze_project_for_rules_calls_tech_stack_detector(issue_detection_rules, mock_tech_stack_detector):
...
def test_detect_dependency_conflicts_python_no_lock_file(issue_detection_rules, mock_tech_stack_detector):
    ...
    assert any(
        issue["type"] == "dependency_conflict" and "lack of strict dependency locking" in issue["description"]
        for issue in issues
    )

def test_detect_dependency_conflicts_javascript_no_lock_file(issue_detection_rules, mock_tech_stack_detector):
    mock_tech_stack_detector.analyze_project.return_value = {
    ...
    assert any(
        issue["type"] == "dependency_conflict" and "missing 'package-lock.json'" in issue["description"]
        for issue in issues
    )

def test_detect_build_failures_python_no_dockerfile(issue_detection_rules, mock_tech_stack_detector):
    mock_tech_stack_detector.analyze_project.return_value = {
        "languages": ["Python"],
    ...
    issues = issue_detection_rules.analyze_project_for_rules("/tmp/python_proj_no_docker")
    assert any(
        issue["type"] == "build_failure_risk" and "No Dockerfile detected" in issue["description"]
        for issue in issues
    )

def test_detect_test_flakiness_pytest_detected(issue_detection_rules, mock_tech_stack_detector):
    mock_tech_stack_detector.analyze_project.return_value = {
        "testing": ["pytest"],
    ...
    assert any(
        issue["type"] == "test_flakiness_risk" and "Potential for test flakiness" in issue["description"]
        for issue in issues
    )

def test_detect_performance_bottlenecks_python(issue_detection_rules, mock_tech_stack_detector):
    mock_tech_stack_detector.analyze_project.return_value = {
        "languages": ["Python"],
    ...
    assert any(
        issue["type"] == "performance_bottleneck_risk" and "asynchronous programming" in issue["description"]
        for issue in issues
    )
...
def test_detect_security_vulnerabilities_javascript(issue_detection_rules, mock_tech_stack_detector):
    ...
    assert any(
        issue["type"] == "security_vulnerability_risk" and "XSS and CSRF" in issue["description"]
        for issue in issues
    )

def test_no_issues_detected_for_empty_stack(issue_detection_rules, mock_tech_stack_detector):
    mock_tech_stack_detector.analyze_project.return_value = {
        "languages": [], "package_managers": [], "frameworks": [],
        "testing": [], "ci_cd": [], "files": [],

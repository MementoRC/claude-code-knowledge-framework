...
@pytest.fixture
def manager(monkeypatch, mock_chroma, mock_semantic_search, mock_pattern_manager, mock_error_solution_manager, mock_pattern_classification, mock_tech_detector):
    # Patch all dependencies
    monkeypatch.setattr("src.uckn.core.organisms.knowledge_manager.ChromaDBConnector", lambda *a, **kw: mock_chroma)
    monkeypatch.setattr("src.uckn.core.organisms.knowledge_manager.SemanticSearch", lambda *a, **kw: mock_semantic_search)
    monkeypatch.setattr("src.uckn.core.organisms.knowledge_manager.PatternManager", lambda *a, **kw: mock_pattern_manager)
    monkeypatch.setattr("src.uckn.core.organisms.knowledge_manager.ErrorSolutionManager", lambda *a, **kw: mock_error_solution_manager)
    monkeypatch.setattr("src.uckn.core.organisms.knowledge_manager.PatternClassification", lambda *a, **kw: mock_pattern_classification)
...
def test_initialization_default(monkeypatch):
    ...
    with patch("src.uckn.core.organisms.knowledge_manager.ChromaDBConnector") as chroma_patch, \
...
def test_search_patterns(manager, mock_pattern_manager):
    ...
    result = manager.search_patterns("query", limit=5, min_similarity=0.8, metadata_filter={"foo": "bar"})
    assert result == [{"id": "pattern-1"}]
...
def test_get_patterns_by_category(manager, mock_pattern_classification):
    ...
    assert result == ["pattern-1"]
    mock_pattern_classification.get_patterns_in_category.assert_called_once_with("cat-1")

def test_get_pattern_categories(manager, mock_pattern_classification):
    mock_pattern_classification.get_categories_for_pattern.return_value = [{"id": "cat-1"}]
    result = manager.get_pattern_categories("pattern-1")
    assert result == [{"id": "cat-1"}]
    mock_pattern_classification.get_categories_for_pattern.assert_called_once_with("pattern-1")
...
def test_error_handling_pattern_manager(monkeypatch):
    ...
    class FailingPatternManager:
        def add_pattern(self, *a, **kw): raise Exception("fail")
        def get_pattern(self, *a, **kw): raise Exception("fail")
        def update_pattern(self, *a, **kw): raise Exception("fail")
        def delete_pattern(self, *a, **kw): raise Exception("fail")
        def search_patterns(self, *a, **kw): raise Exception("fail")
    with patch("src.uckn.core.organisms.knowledge_manager.PatternManager", FailingPatternManager), \
         patch("src.uckn.core.organisms.knowledge_manager.ChromaDBConnector") as chroma_patch, \
...
def test_error_handling_error_solution_manager(monkeypatch):
    class FailingErrorSolutionManager:
        def add_error_solution(self, *a, **kw): raise Exception("fail")
        def get_error_solution(self, *a, **kw): raise Exception("fail")
        def search_error_solutions(self, *a, **kw): raise Exception("fail")
    with patch("src.uckn.core.organisms.knowledge_manager.ErrorSolutionManager", FailingErrorSolutionManager), \
         patch("src.uckn.core.organisms.knowledge_manager.ChromaDBConnector") as chroma_patch, \
         patch("src.uckn.core.organisms.knowledge_manager.SemanticSearch"), \
         patch("src.uckn.core.organisms.knowledge_manager.PatternManager"), \
         patch("src.uckn.core.organisms.knowledge_manager.PatternClassification"), \

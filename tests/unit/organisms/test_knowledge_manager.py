...
def mock_unified_db():
    ...
    db.search_error_solutions.return_value = [{"id": "sol-1", "document": "found solution"}]
    db.add_team_access.return_value = "access-1"
    db.get_team_access.return_value = {"id": "access-1", "user_id": "user1", "project_id": "proj-1", "role": "admin"}
    db.update_team_access.return_value = True
    db.delete_team_access.return_value = True
    db.get_team_access_for_project.return_value = [{"id": "access-1", "user_id": "user1"}]
    db.add_compatibility_entry.return_value = "comp-1"
    db.get_compatibility_entry.return_value = {"id": "comp-1", "source_tech": "Python", "target_tech": "Django"}
    db.update_compatibility_entry.return_value = True
    db.delete_compatibility_entry.return_value = True
    db.search_compatibility_entries.return_value = [{"id": "comp-1", "source_tech": "Python"}]
    db.search_patterns_by_metadata.return_value = [{"id": "pattern-approved", "status": "approved"}]
    return db
...
def manager(monkeypatch, mock_unified_db, mock_semantic_search, mock_tech_detector):
    ...
    monkeypatch.setattr("src.uckn.core.organisms.knowledge_manager.UnifiedDatabase", lambda *a, **kw: mock_unified_db)
    monkeypatch.setattr("src.uckn.core.organisms.knowledge_manager.SemanticSearch", lambda *a, **kw: mock_semantic_search)
    monkeypatch.setattr("src.uckn.core.organisms.knowledge_manager.TechStackDetector", lambda *a, **kw: mock_tech_detector)
...
def test_initialization_default(monkeypatch):
    ...
    with patch("src.uckn.core.organisms.knowledge_manager.UnifiedDatabase") as unified_db_patch, \
         patch("src.uckn.core.organisms.knowledge_manager.SemanticSearch") as search_patch, \
...
def test_add_project(manager, mock_unified_db):
    ...
    mock_unified_db.add_project.assert_called_once_with(project_data["name"], project_data["description"])
...
def test_add_pattern(manager, mock_unified_db, mock_semantic_search):
    pattern_data = {"document": "This is a test pattern.", "metadata": {"foo": "bar"}, "project_id": "proj-1"}
    mock_semantic_search.encode.return_value = [0.1, 0.2, 0.3] # Ensure embedding is returned
...
def test_update_pattern(manager, mock_unified_db, mock_semantic_search):
    updates = {"document": "updated document", "metadata": {"foo": "baz"}, "project_id": "proj-1"}
...
def test_search_patterns(manager, mock_unified_db, mock_semantic_search):
    ...
    result = manager.search_patterns(query, limit=limit, min_similarity=min_similarity, metadata_filter=metadata_filter)
...
def test_update_category(manager, mock_unified_db):
    ...
    assert result is True
    mock_unified_db.update_category.assert_called_once_with("cat-1", {"name": "new", "description": "desc"})
...
def test_assign_pattern_to_category(manager, mock_unified_db):
    ...
    assert result is True
    mock_unified_db.assign_pattern_to_category.assert_called_once_with("pattern-1", "cat-1")

def test_remove_pattern_from_category(manager, mock_unified_db):
    result = manager.remove_pattern_from_category("pattern-1", "cat-1")
    assert result is True
    mock_unified_db.remove_pattern_from_category.assert_called_once_with("pattern-1", "cat-1")
...
def test_add_error_solution(manager, mock_unified_db, mock_semantic_search):
    solution_data = {"document": "This is a test solution.", "metadata": {"foo": "bar"}, "project_id": "proj-1"}
    mock_semantic_search.encode.return_value = [0.1, 0.2, 0.3] # Ensure embedding is returned
...
def test_search_error_solutions(manager, mock_unified_db, mock_semantic_search):
    ...
    metadata_filter = {"foo": "bar"}
    mock_semantic_search.encode.return_value = [0.7, 0.8, 0.9] # Mock query embedding
    result = manager.search_error_solutions(error_query, limit=limit, min_similarity=min_similarity, metadata_filter=metadata_filter)
...
def test_get_team_access(manager, mock_unified_db):
    ...
    assert result == {"id": "access-1", "user_id": "user1", "project_id": "proj-1", "role": "admin"}
...
def test_add_compatibility_entry(manager, mock_unified_db):
    result = manager.add_compatibility_entry("Python", "Django", 0.9, "Good compatibility")
    assert result == "comp-1"
    mock_unified_db.add_compatibility_entry.assert_called_once_with("Python", "Django", 0.9, "Good compatibility")
...
def test_update_compatibility_entry(manager, mock_unified_db):
    ...
    assert result is True
    mock_unified_db.update_compatibility_entry.assert_called_once_with("comp-1", updates)
...
def test_search_compatibility_entries(manager, mock_unified_db):
    ...
    mock_unified_db.search_compatibility_entries.assert_called_once_with("Python", None, None, None)
...
def test_get_all_patterns_by_status(manager, mock_unified_db):
    ...
    mock_unified_db.search_patterns_by_metadata.assert_called_once_with({"status": status})
...
def test_health_status_unavailable(monkeypatch):
    with patch("src.uckn.core.organisms.knowledge_manager.UnifiedDatabase") as unified_db_patch, \
        ...
        assert status["unified_db_available"] is False # Changed from chromadb_available
...
def test_error_handling_unified_db(monkeypatch):
    ...
    class FailingUnifiedDatabase:
        ...
        def is_available(self): return True # Must be available for other methods to be called
        def add_project(self, *a, **kw): raise Exception("fail add_project")
        def get_project(self, *a, **kw): raise Exception("fail get_project")
        def update_project(self, *a, **kw): raise Exception("fail update_project")
        def delete_project(self, *a, **kw): raise Exception("fail delete_project")
        def get_all_projects(self, *a, **kw): raise Exception("fail get_all_projects")
        def add_pattern(self, *a, **kw): raise Exception("fail add_pattern")
        def get_pattern(self, *a, **kw): raise Exception("fail get_pattern")
        def update_pattern(self, *a, **kw): raise Exception("fail update_pattern")
        def delete_pattern(self, *a, **kw): raise Exception("fail delete_pattern")
        def search_patterns(self, *a, **kw): raise Exception("fail search_patterns")
        def add_category(self, *a, **kw): raise Exception("fail add_category")
        def get_category(self, *a, **kw): raise Exception("fail get_category")
        def update_category(self, *a, **kw): raise Exception("fail update_category")
        def delete_category(self, *a, **kw): raise Exception("fail delete_category")
        def assign_pattern_to_category(self, *a, **kw): raise Exception("fail assign_pattern_to_category")
        def remove_pattern_from_category(self, *a, **kw): raise Exception("fail remove_pattern_from_category")
        def get_patterns_by_category(self, *a, **kw): raise Exception("fail get_patterns_by_category")
        def get_pattern_categories(self, *a, **kw): raise Exception("fail get_pattern_categories")
        def add_error_solution(self, *a, **kw): raise Exception("fail add_error_solution")
        def get_error_solution(self, *a, **kw): raise Exception("fail get_error_solution")
        def search_error_solutions(self, *a, **kw): raise Exception("fail search_error_solutions")
        def add_team_access(self, *a, **kw): raise Exception("fail add_team_access")
        def get_team_access(self, *a, **kw): raise Exception("fail get_team_access")
        def update_team_access(self, *a, **kw): raise Exception("fail update_team_access")
        def delete_team_access(self, *a, **kw): raise Exception("fail delete_team_access")
        def get_team_access_for_project(self, *a, **kw): raise Exception("fail get_team_access_for_project")
        def add_compatibility_entry(self, *a, **kw): raise Exception("fail add_compatibility_entry")
        def get_compatibility_entry(self, *a, **kw): raise Exception("fail get_compatibility_entry")
        def update_compatibility_entry(self, *a, **kw): raise Exception("fail update_compatibility_entry")
        def delete_compatibility_entry(self, *a, **kw): raise Exception("fail delete_compatibility_entry")
        def search_compatibility_entries(self, *a, **kw): raise Exception("fail search_compatibility_entries")
        def search_patterns_by_metadata(self, *a, **kw): raise Exception("fail search_patterns_by_metadata")

    with patch("src.uckn.core.organisms.knowledge_manager.UnifiedDatabase", FailingUnifiedDatabase), \
         patch("src.uckn.core.organisms.knowledge_manager.SemanticSearch") as search_patch, \
         patch("src.uckn.core.organisms.knowledge_manager.PatternManager"), \
         patch("src.uckn.core.organisms.knowledge_manager.ErrorSolutionManager"), \
         patch("src.uckn.core.organisms.knowledge_manager.PatternClassification"), \
         patch("src.uckn.core.organisms.knowledge_manager.TechStackDetector"):
        search_patch.return_value.is_available.return_value = True
        search_patch.return_value.encode.return_value = [0.1, 0.2, 0.3] # Mock encode for calls that need it
        km = KnowledgeManager()

...
def test_add_project(pg_connector):
    ...
    added_id = pg_connector.add_record(Project, {"id": project_id, "name": name, "description": description})
...
def test_update_project(pg_connector):
    project_id = str(uuid.uuid4())
    pg_connector.add_record(Project, {"id": project_id, "name": "Old Name", "description": "Old Desc"})
    ...
    assert retrieved_project["description"] == "Old Desc" # Description should be unchanged
...
def test_add_pattern(pg_connector):
    project_id = pg_connector.add_record(Project, {"id": str(uuid.uuid4()), "name": "Pattern Project"})
...
def test_add_error_solution(pg_connector):
    project_id = pg_connector.add_record(Project, {"id": str(uuid.uuid4()), "name": "Error Project"})
    solution_id = str(uuid.uuid4())
    doc_text = "Error: File not found."
    metadata = {"error_category": "IO", "resolution_steps": "Check path", "avg_resolution_time": 15.5}

    added_id = pg_connector.add_record(ErrorSolution, {
        "id": solution_id,
        "project_id": project_id,
        "document_text": doc_text,
        "metadata_json": metadata,
...
def test_add_pattern_category(pg_connector):
    ...
    added_id = pg_connector.add_record(PatternCategory, {"id": category_id, "name": name})
...
def test_assign_pattern_to_category(pg_connector):
    project_id = pg_connector.add_record(Project, {"id": str(uuid.uuid4()), "name": "Proj1"})
    pattern_id = pg_connector.add_record(Pattern, {"id": str(uuid.uuid4()), "project_id": project_id, "document_text": "Pattern A", "metadata_json": {}})
    category_id = pg_connector.add_record(PatternCategory, {"id": str(uuid.uuid4()), "name": "Category X"})
...
def test_remove_pattern_from_category(pg_connector):
    project_id = pg_connector.add_record(Project, {"id": str(uuid.uuid4()), "name": "Proj2"})
    pattern_id = pg_connector.add_record(Pattern, {"id": str(uuid.uuid4()), "project_id": project_id, "document_text": "Pattern B", "metadata_json": {}})
    category_id = pg_connector.add_record(PatternCategory, {"id": str(uuid.uuid4()), "name": "Category Y"})
...
def test_filter_records(pg_connector):
    ...
    proj1_id = pg_connector.add_record(Project, {"id": str(uuid.uuid4()), "name": "Filter Project 1"})
    proj2_id = pg_connector.add_record(Project, {"id": str(uuid.uuid4()), "name": "Filter Project 2"})

    pg_connector.add_record(Pattern, {
        "id": str(uuid.uuid4()), "project_id": proj1_id, "document_text": "Doc A",
        "metadata_json": {"tech": "Python"}, "technology_stack": "Python"
    })
    pg_connector.add_record(Pattern, {
        "id": str(uuid.uuid4()), "project_id": proj1_id, "document_text": "Doc B",
        "metadata_json": {"tech": "Java"}, "technology_stack": "Java"
    })
    pg_connector.add_record(Pattern, {
        "id": str(uuid.uuid4()), "project_id": proj2_id, "document_text": "Doc C",
        "metadata_json": {"tech": "Python"}, "technology_stack": "Python"
    })

    python_patterns = pg_connector.filter_records(Pattern, {"technology_stack": "Python"})
...
def test_team_access_crud(pg_connector):
    project_id = pg_connector.add_record(Project, {"id": str(uuid.uuid4()), "name": "Access Project"})
    user_id = "user123"
    role = "admin"
    access_id = str(uuid.uuid4())

    added_id = pg_connector.add_record(TeamAccess, {"id": access_id, "user_id": user_id, "project_id": project_id, "role": role})
...
def test_compatibility_matrix_crud(pg_connector):
    ...
    added_id = pg_connector.add_record(CompatibilityMatrix, {
        "id": entry_id, "source_tech": source_tech, "target_tech": target_tech,
        "compatibility_score": score, "notes": notes
    })
    ...
    updated = pg_connector.update_record(CompatibilityMatrix, entry_id, {"compatibility_score": 0.9})
    assert updated
    assert pg_connector.get_record(CompatibilityMatrix, entry_id)["compatibility_score"] == 0.9

...
try:
    ...
    logging.getLogger(__name__).warning(
        "SemanticSearchEngine not found. Semantic search capabilities will be limited."
    )
...
class SemanticSearch:
    ...
    def __init__(self, knowledge_dir: str = ".uckn/knowledge"):
        ...
        if SEMANTIC_SEARCH_ENGINE_AVAILABLE and SearchEngineClass:
            ...
            self._logger.warning(
                "SemanticSearchEngine not available, semantic encoding/search will be disabled."
            )
    ...
    def is_available(self) -> bool:
        ...
        if (
            hasattr(uckn.core, "SENTENCE_TRANSFORMERS_AVAILABLE")
            and not uckn.core.SENTENCE_TRANSFORMERS_AVAILABLE
        ):
            return False
    ...
    def encode(self, text: str) -> list[float] | None:
        ...
        try:
            ...
            if (
                self.engine
                and hasattr(self.engine, "sentence_model")
                and self.engine.sentence_model
            ):
                embedding = self.engine.sentence_model.encode(
                    text, convert_to_numpy=True
                )
                return embedding.tolist()
            else:
                self._logger.error(
                    "Sentence transformer model not loaded in SemanticSearchEngine."
                )
                return None
    ...
    def search(
        ...
        if not self.is_available():
            self._logger.warning(
                "Semantic search engine not available, cannot perform search."
            )
            return []
        try:
    ...
    def _extract_text_for_embedding(self, session_data: dict[str, Any]) -> str:
        ...
        if not self.engine:
            self._logger.warning(
                "Semantic search engine not available, cannot extract text."
            )
            return ""
    ...
    def get_embedding_stats(self) -> dict[str, Any]:
        ...
        if not self.engine:
            self._logger.warning(
                "Semantic search engine not available, cannot get stats."
            )
            return {
                "total_embeddings": 0,
                "storage_type": "none",
    ...
    def search_similar_sessions(
        ...
        if not self.engine:
            self._logger.warning(
                "Semantic search engine not available, cannot search sessions."
            )
            return []
        return self.engine.search_similar_sessions(
    ...
    def store_session_embedding(
        ...
        if not self.engine:
            self._logger.warning(
                "Semantic search engine not available, cannot store embedding."
            )
            return False
        return self.engine.store_session_embedding(session_id, session_data)

    def _store_embedding_numpy(
        self, session_id: str, embedding: Any, session_data: dict[str, Any]
    ) -> None:
        ...
        if hasattr(self.engine, "_store_embedding_numpy"):
            return self.engine._store_embedding_numpy(
                session_id, embedding, session_data
            )
        else:

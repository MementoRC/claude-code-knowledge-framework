...
class ErrorSolutionManager:
    ...
    def get_error_solution(self, solution_id: str) -> dict[str, Any] | None:
        ...
        if not self.chroma_connector.is_available():
            self._logger.warning(
                "ChromaDB not available, cannot retrieve error solution."
            )
            return None
    ...
    def search_error_solutions(
        ...
        if not self.chroma_connector.is_available():
            self._logger.warning(
                "ChromaDB not available, cannot search error solutions."
            )
            return []
        if not self.semantic_search.is_available():
            self._logger.warning(
    ...
    def update_error_solution(self, solution_id: str, updates: dict[str, Any]) -> bool:
        ...
        if not self.chroma_connector.is_available():
            self._logger.warning(
                "ChromaDB not available, cannot update error solution."
            )
            return False
    ...
    def delete_error_solution(self, solution_id: str) -> bool:
        ...
        if not self.chroma_connector.is_available():
            self._logger.warning(
                "ChromaDB not available, cannot delete error solution."
            )
            return False

...
async def initiate_pattern_review(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to initiate review: {str(e)}")
...
async def submit_pattern_review_feedback(
    ...
    workflow_manager: WorkflowManager = Depends(get_workflow_manager),
    user_id: str = Depends(get_current_user_id) # Ensure reviewer_id matches user_id or user has permission
):
    """
    ...
    if request.reviewer_id != user_id and "admin" not in get_current_user_roles():
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="User not authorized to submit feedback for this reviewer ID.")
    try:
...
async def transition_pattern_state(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to transition state: {str(e)}")
...
async def get_patterns_awaiting_review_endpoint(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to retrieve pending reviews: {str(e)}")
